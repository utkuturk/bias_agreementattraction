...
#### DATA -----
# Call data
data_ungrammaticalBias <- read.csv("Data/Experiment3_ugb/results/results", #location
...
# give columns names
colnames(data_ungrammaticalBias) = c("Time", "MD5", "ControllerType",  #names for columns
                                   "SentenceNoInStimFile", "Element", "exp_condition", 
                                   "item", "Sentence", "Question","Answer", "RT")
# create subject ids
subject_id <- with(data_ungrammaticalBias, { as.integer(as.factor(paste(Time, MD5))) })
data_ungrammaticalBias$subject <- sprintf("S[%d]", subject_id + 0)

# extract form and the the real data
df_forms_ungrammaticalBias <- data_ungrammaticalBias %>% #extract form related stuff
  subset(ControllerType != "DashedAcceptabilityJudgment" ) %>% # by specifying not dashedacceptability
  gdata::drop.levels()
data_ungrammaticalBias %<>% subset(ControllerType == "DashedAcceptabilityJudgment") 
data_ungrammaticalBias$subject %<>% as.factor()
data_ungrammaticalBias$exp_condition %<>% as.factor()
data_ungrammaticalBias$RT %<>% as.integer()
#extract ages and native language
age <- df_forms_ungrammaticalBias %>% dplyr::filter(Sentence == "age") %>% 
  dplyr::select(subject, age = Question)
natturk <- df_forms_ungrammaticalBias %>% dplyr::filter(Sentence == "natturk") %>% 
...
rows_stim_ungrammaticalBias <- data_ungrammaticalBias[c(T,F),]
rows_resp_ungrammaticalBias <- data_ungrammaticalBias[c(F,T),]
stopifnot( all(is.na( rows_stim_ungrammaticalBias$RT )) )

#check for practice accuracy
data_ungrammaticalBias_practice <- rows_resp_ungrammaticalBias %>% subset(exp_condition == "practice") 
data_ungrammaticalBias_practice %<>% 
  left_join(forms_ungrammaticalBias) %>% 
...
data_ungrammaticalBias_practice %>% subset(Response == "NULL")
...
data_ungrammaticalBias_nobadpractice <- rows_resp_ungrammaticalBias_nobadpractice %>% 
  left_join(forms_ungrammaticalBias_nobadpractice) %>% 
  dplyr::select(-MD5, -Time, -ControllerType, -Sentence, -Element) %>%
  dplyr::rename(ResponseCorrect=Answer, Response=Question) %>%
  dplyr::select(-ResponseCorrect) %>% subset(exp_condition != "practice")

# add trial no
data_ungrammaticalBias_nobadpractice %<>% group_by(subject) %>% mutate(trial_no = seq(subject))
#identify late responses
data_ungrammaticalBias_nobadpractice %<>% mutate( late_response = (Response == "NULL"), 
...
data_ungrammaticalBias_nobadpractice$Response %<>% as.character() %>% enc2native()
stopifnot( all(data_ungrammaticalBias_nobadpractice$Response %in% responses | is.na(data_ungrammaticalBias_nobadpractice$Response) ) )

data_ungrammaticalBias_nobadpractice$response_yes <- ifelse(grepl("P'ye",data_ungrammaticalBias_nobadpractice$Response) , T, 
                                                          ifelse(grepl("Q'ya",data_ungrammaticalBias_nobadpractice$Response) , F, NA))
print( with(data_ungrammaticalBias_nobadpractice, table(Response, response_yes)) )
data_ungrammaticalBias_nobadpractice %<>% dplyr::select(-Response)
